...
| Text overlay (static)   | **CoreText** (iOS) / native `StaticLayout` + `Canvas` rasterization, composited via the transcoder GL overlay path (Android) | No Skia dep; minimal declarative API; cross-platform parity is "visually similar," not pixel-identical      |
...
│   │   │   ├── OverlayRenderer.mm                    # CIFilter + CoreText
...
| `Video.thumbnails`       | Probe                             | Batch N JPEGs from one decode session (filmstrips/scrubbers), optional resize + seek tolerance                                          |
...
| `Overlay.Text`           | Spec primitive                    | Static text overlay; rendered via CoreText / Media3 `TextOverlay`. **Visually similar, not pixel-identical** across platforms.        |
...
- **Skia — zero in the library.** Neither C++ nor JS. Static image/text overlays use platform-native primitives (CIFilter / CoreText on iOS, Media3 `BitmapOverlay` / `TextOverlay` on Android). Skia enters only when a consumer opts into the compose worklet path and brings in `@shopify/react-native-skia` themselves.
...
- **Multi-track / PiP overlay tracks** (`clip.track > 0`) composite on both platforms — iOS via `AVMutableVideoComposition` ([#17](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/17)), Android via a Media3 multi-sequence `Composition` + `VideoCompositorSettings` ([#45](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/45), see [`rendering-android.md`](./rendering-android.md)). v1 drops overlay-track audio on both; the base track honours all `audio.mode`s, and spec-level overlays (watermarks) compose on top of the composite output on both platforms — on Android via a parallel audio-only sequence and a composition-level `OverlayEffect` respectively ([#52](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/52)). A **base-track overlap** combined with PiP overlay tracks is handled in two passes on Android (crossfade the base to a temp, then composite the overlays on top), at parity with iOS — fully closing #52. **Timeline overlaps** (crossfade) compose on both platforms — iOS via `AVMutableVideoComposition` ([#18](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/18)), Android via a two-sequence ping-pong Media3 `Composition` with a time-ramped alpha + per-clip audio volume ramp ([#43](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/43)).
