...
- **Multi-track / PiP overlay tracks** (`clip.track > 0`) composite on both platforms — iOS via `AVMutableVideoComposition` ([#17](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/17)), Android via a Media3 multi-sequence `Composition` + `VideoCompositorSettings` ([#45](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/45), see [`rendering-android.md`](./rendering-android.md)). v1 drops overlay-track audio on both; the base track honours all `audio.mode`s, and spec-level overlays (watermarks) compose on top of the composite output on both platforms — on Android via a parallel audio-only sequence and a composition-level `OverlayEffect` respectively ([#52](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/52)). The one Android gap left from #52: a **base-track overlap** combined with PiP overlay tracks still rejects. **Timeline overlaps** (crossfade) compose on both platforms — iOS via `AVMutableVideoComposition` ([#18](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/18)), Android via a two-sequence ping-pong Media3 `Composition` with a time-ramped alpha + per-clip audio volume ramp ([#43](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/43)).
