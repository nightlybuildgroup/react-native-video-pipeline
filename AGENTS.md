...
- Android instrumented suite (`connectedDebugAndroidTest`) needs a booted emulator (no host fast-path); gate Android Kotlin edits locally with the offline compile of **both** source sets (`compileDebugKotlin` + `compileDebugAndroidTestKotlin --offline`). CI runs the full instrumented suite on an API 36 + 24 emulator matrix via `.github/workflows/android-instrumented.yml`.
