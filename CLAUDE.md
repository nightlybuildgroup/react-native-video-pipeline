...
- Android instrumented suite (`connectedDebugAndroidTest`) — the only thing that exercises the real Media3 encode/composite pipeline; needs a booted emulator, so it has no host fast-path. Locally, gate Android Kotlin edits with the offline compile loop (`./gradlew :react-native-video-pipeline:compileDebugKotlin :react-native-video-pipeline:compileDebugAndroidTestKotlin --offline`, both source sets), then run the full suite against a **booted local emulator** before landing Android changes — that's the gate, not CI. The [`.github/workflows/android-instrumented.yml`](./.github/workflows/android-instrumented.yml) workflow can run the same suite on a GitHub-hosted API 36 + 26 emulator matrix, but it's **disabled in CI** (manual `workflow_dispatch` only) because the per-run emulator boot is too slow/expensive to gate every push/PR — see [#56](https://github.com/nightlybuildgroup/react-native-video-pipeline/issues/56) (closed won't-do). If you do dispatch it, API 36 is the load-bearing leg (it caught the #49 encoder coded-vs-displayed dimension bug class) and the lower leg is 26, not 24, because the bare-example gradle root that assembles the test APK pins minSdk 26.
